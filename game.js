    ...
    .map((s) => {
      ...
      return `
        ...
        .toUpperCase()}</span>
                ...
                <span class="text-right text-orange-400" title="$${
                  s.config.cost
...
function updateFinancesDisplay() {
  ...
  if (incomeDetails) {
    ...
    incomeTypes.forEach((t) => {
      ...
      if (value > 0 || count > 0) {
        hasIncome = true;
        incomeHtml += `<div class="grid grid-cols-4 gap-1"><span class="${
          t.color
        }">${
          t.label
  ...
  if (netProfitEl) {
    ...
    netProfitEl.className = `text-right font-bold ${
  ...
  if (
    ...
    document.getElementById("modal-desc").innerHTML = `
            <div class="text-left space-y-3">
                <div class="text-center text-2xl font-bold text-yellow-400 mb-2">Final Score: ${
                  STATE.score.total
                }</div>
